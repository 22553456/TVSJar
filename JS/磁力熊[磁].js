	...
	推荐: `js:
		...
		list.forEach(it => {
			d.push({
				...
				// pic_url: pd(it, '.card-img&&style'), // 有图片
				pic_url: /!'/.test(pd(it, '.card-img&&style'))?pd(it, '.card-img&&style'):pd(it, '.card-img&&style').replaceAll("'",""), // 兼容 其它壳子
				url: pd(it, 'a&&href')
			});
		})
		setResult(d);
	...
	一级: `js:
		...
		list.forEach(it => {
			d.push({
				title: pdfh(it, 'h2&&Text'),
				desc: pdfh(it, '.me-auto&&Text') + '分 / ' + pdfh(it, '.small&&Text'),
				// pic_url: pdfh(it, '.card-img&&style'), // 有图片
				pic_url: /!'/.test(pd(it, '.card-img&&style'))?pd(it, '.card-img&&style'):pd(it, '.card-img&&style').replaceAll("'",""), // 兼容其它壳子
				url: pd(it, 'a&&href')
			});
		})
		setResult(d);
